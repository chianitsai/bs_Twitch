...
if plot_polLoc_speed
    ...
    for strain = 1:1:nbr_strains
        ...
        hold on
        ylabel(strcat("Polar Loc Mot Index (",type_ratio,")"))
        xlabel("Speed (µm/s)")
        ...
        if rep_colour
            for rep = 1:1:nbr_replicates
                ...
                if ~isempty(polar_loc_speed_motile_results{index_type(rep),3})
                    plot([polar_loc_speed_motile_results{index_type(rep),3}{:,5}],[polar_loc_speed_motile_results{index_type(rep),3}{:,type_ratio_position(1)}],marker_rep,'MarkerSize',5,'Linewidth',1) % plots median speed of the replicate
                end
            end
        else
        ...
        graph_type = strcat('polLoc_vs_speed_',num2str(sscanf(type,'%i')),'_',type_ratio);
        saveas(gcf,strcat(save_dir,save_name,graph_type,'.jpg'));
        saveas(gcf,strcat(save_dir,'fig_files\',save_name,graph_type,'.fig'));
        saveas(gcf,strcat(save_dir,'svg_files\',save_name,graph_type,'.svg'));
    end    
