...
for nbr=1:1:nbr_bact
    ...
    sum_negative=sum(v_dot_x<0); % this summs up the logical value of where v_dot_x is smaller than 0, so the variable will always be positive
    sum_positive=sum(v_dot_x>0); % this summs up the logical value of where v_dot_x is bigger than 0, so the variable will always be positive
